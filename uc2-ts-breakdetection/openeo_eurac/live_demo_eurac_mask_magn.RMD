...
Plot the NDVI - red line = event date, blue line = r4openeo detection

```{r, plot_ndvi, message=FALSE, warning=FALSE}
event_date = "2018-10-27"
break_date = brks[st_transform(x = pnt, crs = st_crs(brks))] %>% pull() %>% c()

plot_px = ggplot(ndvi_px_df, aes(x = dates, y = value)) + 
  geom_line() + 
  geom_point() + 
  geom_vline(xintercept = as.Date(event_date), col = "red") +
  geom_vline(xintercept = as.Date(lubridate::date_decimal(break_date)), col = "blue")
...
```{r, map_brks}
mapview(brks) + mapview(brks_eco) + mapview(pnt)
```
...
Plot break timings - red line = event date of the vaja storm, blue line = r4openEO break, green line = eco4alps break

```{r, plot_brk_timings, message=FALSE, warning=FALSE}
break_date_eco = brks_eco[st_transform(pnt, crs = st_crs(brks_eco))] %>% pull() %>% c()
...
brks_boxplot = ggplot(brks_all_v, aes(x = source, y = brks, group = source)) + 
  ...
  geom_hline(yintercept = break_date, col = "blue") +
  geom_hline(yintercept = break_date_eco, col = "green") +
...
And as histogram - red line = event date of the vaja storm, blue line = r4openEO break, green line = eco4alps break
...
plt_hist_lags = ggplot(brks_all_v, aes(x=brks)) + 
  ...
  geom_vline(xintercept = break_date, color = "blue") +
  geom_vline(xintercept = break_date_eco, color = "green") +
  facet_wrap(~ source)
...
Plot the two NDVI time series - red line = event date of the vaja storm, blue line = r4openEO break, green line = eco4alps break
...
plot_px2 = ggplot(ndvi_all_df, aes(x = dates, y = value, col = src)) + 
  ...
  geom_vline(xintercept = as.Date(event_date), col = "red") + 
  geom_vline(xintercept = as.Date(lubridate::date_decimal(break_date)), col = "blue") +
  geom_vline(xintercept = as.Date(lubridate::date_decimal(break_date_eco)), col = "green")
